      ...
      <div className="w-full max-w-7xl animate-fade-in-up">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-24 items-center">
          ...
          <div className="space-y-8 max-w-xl">
            <h1 className={`text-4xl md:text-5xl leading-tight tracking-tight text-foreground animate-fade-in-up [animation-delay:200ms] ${geistSans.className}`}>
            Why Choose Brandora
            ...
            <div className="flex flex-wrap gap-4 animate-fade-in-up [animation-delay:600ms]">
              ...
              <a href="https://calendly.com/nandaniramchandani3/30min?month=2025-11" target="_blank" rel="noopener noreferrer" className="group relative px-6 py-3  rounded-md font-medium overflow-hidden transition-all duration-300 hover:scale-105 hover:shadow-lg border border-border">
            ...
            <div className="relative w-full h-full">
              <VerticalMarquee speed={20} className="h-full">
                {marqueeItems.map((item, idx) => (
                  <div
                    key={idx}
                    className={`text-2xl md:text-5xl   tracking-tight py-8 marquee-item ${jetBrainsMono.className}`}
                  >
                    {item}
                  </div>
                ))}
